...
export function Requirements({project, route}) {
  ...
  return <>
    <PageTitle title="需求管理" subtitle="维护故事角色、优先级、可测验收条件与不可覆盖的版本记录。" actions={writable && <PrimaryButton onClick={() => setEditor({mode: 'create'})}>新建需求</PrimaryButton>}/>
    ...
    <div className="split s1-split">
      ...
      <Card className="req-detail"><ResourceState resource={detail}>{selected ? <>
        ...
        <h2>{selected.title}</h2><div className="tagline"><Tag tone={priorityTone[selected.priority]}>{selected.priority}</Tag><Tag>来源 {selected.source}</Tag><Tag tone={selected.story_role ? 'purple' : 'orange'}>{selected.story_role ? `故事角色 ${selected.story_role}` : '故事角色待确认'}</Tag><Tag tone="green">版本 {selected.version}</Tag></div>
        ...
        {can(project, 'requirement.history') && <><SectionHead title="版本历史"/><div className="version-history">{detail.data.versions.map(version => <details key={version.version} open={version.version === selected.version}><summary><b>v{version.version}</b><span>{version.reason}</span></summary><p>{version.changed_by_name}{version.changed_at ? ` · ${dateTime(version.changed_at)}` : ''}</p><p>{version.title} · {version.priority} · {version.story_role ? `角色 ${version.story_role}` : '角色待确认'}</p><p className="long-text">{version.acceptance_criteria}</p></details>)}</div></>}
...
function RequirementForm({project, requirement, onClose, onSaved}) {
  ...
  return <Modal title={requirement ? `编辑需求 · ${requirement.id}` : '新建需求'} onClose={onClose}><form onSubmit={submit}>
    ...
    <Field label="故事角色"><input name="story_role" defaultValue={requirement?.story_role || ''} maxLength={200} placeholder="例如：产品负责人、项目管理员、执行成员"/></Field>
